$(function(){
    //define the array that contains the tags
    let myArray   = ["your_tag"], 
    // define the only input in the page
    input     = $(".input"),  

        addTag    = (text) => 
        { 
        // the add tag function
        input.before(`<span class='tag' data-value='${text}'>${text}<i class="fa fa-times-circle"></i></span>`);
        
        // to reset the max chars after adding the tag
        charsLengthSpan.html(maxChars); 
        },

        removeTag = (text, removeStyle = "hard" ) => 
        { 
            //the remove tag function
            ...
            if(removeStyle == "hard") 
            {
                ...
                $(".tags-and-input").find(`.tag[data-value='${text}']`).remove(); 
            } 
        ...
        $(".add_tags").on("click", function() {
            if(myArray.length == 0) {
               ...
               result += "]";
               result += `<span><i class="fa fa-plus"></i></span>`;  //this is a future element
